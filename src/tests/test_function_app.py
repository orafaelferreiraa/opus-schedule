...
def test_hybrid_judge_content_score_drives_ranking(monkeypatch):
    """A Judge hybrid (source=llm) anexa _content_score e o plano ranqueia por ele:
    o corte curto julgado ótimo vence o corte longo julgado fraco."""
    _patch_telemetry(monkeypatch)

    class TwoClipClient(FakeClient):
        def get_clips_by_collection(self, _collection_id):
            return [
                {"id": "P1.longo_raso", "projectId": "P1", "title": "Longo raso", "durationMs": 90000},
                {"id": "P1.curto_otimo", "projectId": "P1", "title": "Curto ótimo", "durationMs": 20000},
            ]

    monkeypatch.setattr(function_app, "OpusClient", TwoClipClient)
    # Judge LLM: aprova ambos, mas o corte curto tem substância muito maior.
    monkeypatch.setattr(
        function_app,
        "judge_clips",
        lambda clips, _settings: [
            {"id": "P1.longo_raso", "decision": "APPROVE", "final_score": 55, "source": "llm"},
            {"id": "P1.curto_otimo", "decision": "APPROVE", "final_score": 92, "source": "llm"},
        ],
    )
    ...
    req = func.HttpRequest(
        method="POST",
        url="http://localhost/api/schedule-existing-clips",
        body=json.dumps({"collection_id": "COL1", "dry_run": True, "judge_mode": "hybrid"}).encode(),
        headers={"Content-Type": "application/json"},
    )
    ...
    assert response.status_code == 200
    order = [item["clipId"] for item in payload["schedule_plan"]["YOUTUBE"]]
    assert order == ["curto_otimo", "longo_raso"], f"Score de conteúdo deveria ordenar; obtido {order}"
